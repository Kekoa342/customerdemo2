...
public class CustomerDbUtil {
	...
	public List<Customer> getCustomers(int queryType) throws Exception {
		...
		try {
			...
			String sql2 = "select first_name, last_name FROM customer where first_name = 'Adrian'";
			String sql3 = "Select * FROM customer where customer_id= 7";
			String sql4= "select * from customer where first_name != 'Rocky'";
			String sql5= "select * from customer where last_name like '%Wil%'";
			String sql6= "select * from customer where first_name = 'Anna' OR first_name= 'Mimi'";
			String sql7= "select * from customer where first_name in ('John', 'Adrian', 'Michael', 'Martin')";
			String sql8= "SELECT customer_id , first_name , last_name ,  CONCAT(first_name, ' ', last_name) as full_name FROM customer ORDER BY customer_id;";
...
public List<Customer> getCustomersBuilder(String finalString) throws Exception {
		...
		try {
			...
			// create sql statement
			myStmt = myConn.createStatement();
			myRs=myStmt.executeQuery(finalString);
			...
			while (myRs.next()) {
				...
				String customerFirstName = myRs.getString("first_name");
				String customerLastName = myRs.getString("last_name");
...
public List<Promotion> getPromotions() throws Exception {
	...
	try {
		...
		while (myRs.next()) {
			...
			String promoStart = myRs.getString("promotion_start");
			String promoEnd = myRs.getString("promotion_end");
